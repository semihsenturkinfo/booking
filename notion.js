  ...
  try {
    if (req.method === 'GET') {
      ...
      if (full) {
        ...
        const bookings = results.map(p => ({
          ...
          status: p.properties['Status']?.select?.name || 'Pending',
        ...
        const bookings = results
          .filter(p => p.properties['Status']?.select?.name !== 'Cancelled')
          .map(p => ({
            date: p.properties['Preferred Shoot Date']?.date?.start || '',
            slot: getText(p.properties['Time']),
    ...
    if (action === 'create') {
      ...
      const response = await fetch(`https://api.notion.com/v1/pages/${pageId}`, {
        ...
        body: JSON.stringify({ properties: { 'Status': { select: { name: 'Confirmed' } } } })
      });
      const data = await response.json();
      if (!response.ok) return res.status(400).json({ error: data });
